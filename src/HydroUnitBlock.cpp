...
void HydroUnitBlock::generate_abstract_variables( Configuration *stvv )
{
 ...
 if( !v_flow_rate.empty() ) {
  ...
  v_flow_rate.resize(boost::extents[f_time_horizon][number_arcs]);
  ...
  for( Index t = 0; t < f_time_horizon; ++t ) {
   for( Index arc = 0; arc < number_arcs; ++arc ) {
     ...
     if (!v_primary_rho.empty()) {
      ...
      linear_function->add_variable( &v_active_power[t][arc], PrimaryRho[t][arc] );

     } else{

      linear_function->add_variable( &v_active_power[t][arc], 0 );
   ...
   for( Index t = 0; t < f_time_horizon; ++t ) {
    for( Index arc = 0; arc < number_arcs; ++arc ) {
      ...
      if (!v_secondary_rho.empty()) {
       ...
       linear_function->add_variable( &v_active_power[t][arc], SecondaryRho[t][arc] );
      } else{
       linear_function->add_variable( &v_active_power[t][arc], 0 );
