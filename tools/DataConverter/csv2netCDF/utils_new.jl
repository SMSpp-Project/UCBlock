...
function profile(d::Dict, profile_name::String)
    profile_block = field_d(d, "profile", Dict())
...
function read_input(file_name::AbstractString)
    ...
    for tariff_name in keys(market_data)
        change_profile!(market_data[tariff_name], opt_data)
    end
