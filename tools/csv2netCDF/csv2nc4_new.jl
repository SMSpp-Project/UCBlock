...
function csvEC2nc4(deterministic::Bool=false)
    ...
    peak_tariff_data = [sum(
                            profile(field(market_data, field(users_data[u], "tariff_name")), "peak_tariff")[w] *
                            profile(field(market_data, field(users_data[u], "tariff_name")), "peak_weight")[w]
                            for u in user_set)
                        for w in peak_set] *
                       sum(1 / ((1 + field(gen_data, "d_rate"))^y) for y in year_set)
    ...
    if !deterministic # stochastic model
        ...
        # The mode "c" stands for creating a new file (clobber)
        tssb_ds = NCDataset(string("../EC_Data/TSSB_EC", middle, "Test", last, ".nc4"), "c", attrib=OrderedDict("SMS++_file_type" => 1))
...
file_name = !isempty(NO_OPTION_ARGS) ?
            ...
            "energy_community_model_new.yml"
...
gen_profiles = field_d(gen_data, "profile", Dict())
