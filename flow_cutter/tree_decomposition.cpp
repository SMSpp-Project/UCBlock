...
void store_tree_decompostion_of_order(ArrayIDIDFunc tail, ArrayIDIDFunc head, const ArrayIDIDFunc&order,
	vector<vector<int>>& tree_bags, vector<int>& start_tree_edges, vector<int>& end_tree_edges){
	...
	for(int i=0; i<bag_count; ++i){
		vector<int> reordered_nodes_in_bag_i;
		for(auto x:nodes_in_bag[i])
			reordered_nodes_in_bag_i.push_back(order(x));
		tree_bags.push_back(reordered_nodes_in_bag_i);
	}

	{
		auto store_backbone_edge = [&](int b, int p){
			start_tree_edges.push_back(b);
			end_tree_edges.push_back(p);
		};
